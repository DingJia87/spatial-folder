...
收纳以逻辑画布右侧内缩 132 点、底部上移 180 点为请求锚点，吸附后通常为右侧内缩约 120 点、底部上移约 176 点。调用 `CanvasLayoutEngine.stackImportedItems`，所有本批新增项目使用同一坐标。

引擎只为整堆寻找一次位置：请求锚点无冲突时直接使用；被旧图标占用时才寻找附近空网格。既有坐标不得改变，没有容量或空位的新增项目进入待放置区。

顶层图标显示同点项目数量角标。拖走顶层项目后，下一项自然露出，角标随坐标分组自动减少。落位后必须把 `OperationCanvasItem` 写入事务记录，使撤销和重做能够恢复对应布局元数据。
