...
3. Treat `Release/5.1.0/`, `Release/5.2.0/`, and the formal 5.3.0 release in `Release/5.3.0/` as frozen usable packages; 5.2.1 and 5.2.2 candidates stay archived in their existing directories. Never overwrite an existing package.
...
- The formal 5.3.0 artifact is `Release/5.3.0/指针空间.app` plus `Release/5.3.0/指针空间.zip`. It is arm64, uses ad-hoc signing, and the ZIP SHA-256 is `506d600de07a636dbec2f1417273afab9b9ce0a34418070e8e41b05d13fc1124`.
...
- The user has accepted and published 5.3.0. The source commit and annotated tag `v5.3.0` are on `main`; GitHub Release `v5.3.0` is Latest and ships `Pointer-Space-5.3.0.zip` with the verified SHA-256 above.
- Never rebuild over `Release/5.3.0/指针空间.app` or its ZIP. If a defect is found, create a clearly named new candidate artifact or bump the version instead of overwriting the formal release.
